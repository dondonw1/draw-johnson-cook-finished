...
def output_excel():
    ...
    # If any input is invalid, return without outputting
    if None in (A, B, C, n, eps_dot, eps_dot_0):
        return
    ...
    # Create a Pandas DataFrame with strain and stress data
    data = {'Strain': eps, 'Stress (MPa)': stress}
    df = pd.DataFrame(data)

    # Save the DataFrame to an Excel file using the curve_name
    if curve_name:  # If curve_name is not empty
        df.to_excel(f'{curve_name}.xlsx', index=False)
        messagebox.showinfo("Output", f"Data saved to {curve_name}.xlsx")
    else:
        df.to_excel('johnson_cook_data.xlsx', index=False)
        messagebox.showinfo("Output", "Data saved to johnson_cook_data.xlsx")
...
entry_n = create_input_field(root, "n", 2, "0.26", n_var)
entry_C = create_input_field(root, "C", 3, "0.014", C_var)
...
# Create buttons for plotting and outputting
tk.Button(root, text="Plot Curve", command=plot_curve).grid(row=7, column=0)
tk.Button(root, text="Output to Excel", command=output_excel).grid(row=7, column=1)
